...
### Fixed
- 🐛 **КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ**: Исправлена работа с 2 множествами (критериями)
  - Проблема: При 2 критериях отображалось только 5-8 элементов вместо всех (например, 8 вместо 80)
  - Причина 1: В расчёт `n1` и `n2` ошибочно включались `n13` и `n23` (которые существуют только при наличии 3-го множества)
  - Причина 2: Алгоритм расчёта размеров сетки мог создавать сетку меньше чем количество элементов
  - Решение 1: Добавлена проверка `hasSet3` для корректного расчёта `n1` и `n2`
  - Решение 2: Добавлено автоматическое увеличение сетки если `gridCapacity < totalItems`
  - **Результат**: Все элементы теперь корректно отображаются для 1, 2 и 3 множеств

### Added
- ✅ Comprehensive unit-тесты для проверки работы с 1, 2 и 3 множествами
  - `test/grid-size-tests.js` - тесты для Node.js
  - `test/grid-size-tests.html` - визуальные тесты в браузере
  - `test/run-tests.js` - раннер для Node.js окружения

### Changed
- Улучшен алгоритм расчёта размеров сетки для гарантии отображения всех элементов
- Удалено жёсткое ограничение размеров сетки размерами canvas (было добавлено в v2.5.0)

### Technical
- Формула расчёта `n1` и `n2` теперь учитывает наличие третьего множества:
  ```javascript
  const hasSet3 = sorted.set3.length > 0;
  const n1 = sorted.set1.length + n12 + (hasSet3 ? n13 : 0) + ...;
  const n2 = sorted.set2.length + n12 + (hasSet3 ? n23 : 0) + ...;
  ```
- Добавлена проверка и автоматическая коррекция размеров сетки
- Все 4 unit-теста успешно проходят

### Impact
- Критичность: **CRITICAL**
- Затронутые версии: v2.0.0 - v2.6.0
- Исправлено критическое поведение для случаев с 2 критериями (наиболее частый сценарий использования)
